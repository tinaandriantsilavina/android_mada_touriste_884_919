...
public class LieuService {
    public static void getAll() throws Exception{
        ...
        LieuInterface lieuInterface = RetrofitClient.getRetrofitInstance(token).create(LieuInterface.class);

        String search_query = "";
        Call<CustomResponse> call = lieuInterface.getAllLieu(search_query);
        call.enqueue(new Callback<CustomResponse>() {
                         @Override
                         public void onResponse(Call<CustomResponse> call, Response<CustomResponse> response) {
                             if (response.body().getStatus() == 200) {
                                 String jsonString = new Gson().toJson(response.body().getDatas());
                                 List<Lieu> lieus = new Gson().fromJson(jsonString, new TypeToken<List<Lieu>>(){}.getType());
    ...
    public static void getByIdprovince() throws Exception{
        String token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2NGMzZjcxODU2ZDVjMzY2MTQ0YzI1MDMiLCJub20iOiJyYWtvdG8iLCJwcmVub20iOiJiZSIsImVtYWlsIjoicmFrb3RvQG1hZGF0b3VyLmNvbSIsImlhdCI6MTY5MDkyNjczOSwiZXhwIjoxNjkxNTMxNTM5fQ.VdC4_qJQe1NOCDAT_tEPW409hAbjb1F-HrmYNvizqkE";
        LieuInterface authInterface = RetrofitClient.getRetrofitInstance(token).create(LieuInterface.class);

        String idprovince = "64c3d9a8e1d443289a3a15fe";
        Call<CustomResponse> call = authInterface.getLieuByIdprovince(idprovince);
        call.enqueue(new Callback<CustomResponse>() {
                         @Override
                         public void onResponse(Call<CustomResponse> call, Response<CustomResponse> response) {
                             if (response.body().getStatus() == 200) {
